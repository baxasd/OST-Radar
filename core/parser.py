...
# TLV type identifier — only the Doppler heatmap is needed
TLV_RANGE_DOPPLER_HEAT_MAP = 5

# Force Little-Endian (<) to prevent struct misalignment
_HEADER_FMT = "<Q8I"
...
def _parse_rdhm(data: bytes, length: int, out: dict):
    """Maps raw bytes directly to a uint16 numpy array."""
    try:
        out["RDHM"] = np.frombuffer(data[:length], dtype=np.uint16).copy()
...
def parse_standard_frame(data: bytes) -> dict:
    ...
    for _ in range(num_tlvs):
        ...
        if tlv_type == TLV_RANGE_DOPPLER_HEAT_MAP:
            _parse_rdhm(data[:tlv_len], tlv_len, out)
            # No other TLV types are needed; bail out early
            break
